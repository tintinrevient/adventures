ATST.prototype = Object.create(Card.prototype);
...
function ATST(){
	...
	//Properties
	this.cardname = "atst"; //used for paths
	this.name="AT-ST";
	this.originalAttack=2;
	this.originalHealth=12;
	this.attack=this.originalAttack;
	this.health=this.originalHealth;
	this.sleep=true;
	this.description="Deals damage both to players and opposing units";
	...
	//Visuals
		...
		this.thumbnail.setTransform(10,10, .095, .1);		
		...
		this.avatar = 
			new createjs.Bitmap(Preloader.pull("material/img/cards/"+ this.cardname +"_thumbnail.png"));
		this.avatar.setTransform(0,0,.05,.086);
...
ATST.prototype.ability = function() {
	if (!this.sleep) {
		for (var i=0; i<Match.players.length; i++) {
			if (i!==Match.activePlayer) {
				Match.players[Match.activePlayer].health -= this.attack;
